import { getKelvinEntity, getOwnerPlayer, getState, sendKelvinMessage, setMode, 
setBlock, getForwardPosition } from "./utils.js";
...
export function prepareBuildTask(player) {
  ...
  for (let dx = 0; dx < 5; dx++) {
    for (let dz = 0; dz < 5; dz++) {
      ...
      const blockId = isCorner ? "minecraft:oak_log" : 
"minecraft:oak_planks";
      for (let dy = 1; dy <= 3; dy++) {
        if (dz === 0 && dx === 2 && dy <= 2) {
          // Door opening
          const airPos = translate(originPos, baseY, forward, right, dx, dz,
  dy);
...
export function buildHutTick() {
  ...
  if (task.workSpot) {
    kelvin.teleport({ ...task.workSpot, y: task.workSpot.y + 1 }, { 
keepVelocity: false });
  }
  for (let i = 0; i < 8 && task.index < task.placements.length; i++, 
task.index++) {
